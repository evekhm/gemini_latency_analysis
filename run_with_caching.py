...
async def run_autonomous_analysis(replay_file_path: str = None):
    ...
    # Set config file path for agent tools
    if replay_file_path:
        os.environ["LATENCY_ANALYSIS_CONFIG_FILE"] = os.path.abspath(replay_file_path)
        print(f"✓ Configuration file set to: {os.environ['LATENCY_ANALYSIS_CONFIG_FILE']}")
    ...
    # Structure: agent_name -> {tool_name: count}
                ...
                if hasattr(event, 'content') and event.content and hasattr(event.content, 'parts'):
                    for part in event.content.parts:
                        if hasattr(part, 'function_call') and part.function_call:
                             ...
                             # Use author if available, else 'unknown'
                             agent_name = event.author if hasattr(event, 'author') and event.author else 'unknown'
                             tool_usage[agent_name][tool_name] += 1
...
*Measurements of the report generation process itself, showing which tools were used by the agents to build this report.*
...
"""
                ...
                for tools in tool_usage.values():
                    total_tool_usage.update(tools)

                for tool, count in total_tool_usage.most_common():
                     summary_md += f"| `{tool}` | {count} |\n"

                summary_md += """
### Tool Usage Statistics (By Agent)
*Breakdown of tool calls by each specific agent.*

| Agent | Tool Name | Count |
| :--- | :--- | :--- |
"""
                # Flatten and sort usage by count (descending)
                usage_list = []
                for agent, tools in tool_usage.items():
                    for tool, count in tools.items():
                        usage_list.append((agent, tool, count))
                
                # Sort by count desc, then agent asc, then tool asc
                usage_list.sort(key=lambda x: (-x[2], x[0], x[1]))

                for agent, tool, count in usage_list:
                    summary_md += f"| `{agent}` | `{tool}` | {count} |\n"
